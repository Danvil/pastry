...
namespace pastry
{
	...
	struct sprite_manager
	{
		...
		void add_sheet(const detail::def_sheet& sheet);
	...
	void sprite_manager::add_sheet(const detail::def_sheet& sheet)
	{
	...
	void sprite_group::update(float t, float dt)
	{
		...
		for(const std::shared_ptr<sprite>& sp : sprites_) {
			...
			sprite_render_group& g = s_sprite_manager->groups_[sheet_tag];
			float u_div = 1.0f / static_cast<float>(g.w);
			float v_div = 1.0f / static_cast<float>(g.h);
			vv.push_back({s.x-s.sx, s.y+s.sy, (st.u_px         )*u_div, (st.v_px+st.sv_px)*v_div});
			vv.push_back({s.x+s.sx, s.y+s.sy, (st.u_px+st.su_px)*u_div, (st.v_px+st.sv_px)*v_div});
			vv.push_back({s.x+s.sx, s.y-s.sy, (st.u_px+st.su_px)*u_div, (st.v_px         )*v_div});
			vv.push_back({s.x-s.sx, s.y+s.sy, (st.u_px         )*u_div, (st.v_px+st.sv_px)*v_div});
			vv.push_back({s.x+s.sx, s.y-s.sy, (st.u_px+st.su_px)*u_div, (st.v_px         )*v_div});
			vv.push_back({s.x-s.sx, s.y-s.sy, (st.u_px         )*u_div, (st.v_px         )*v_div});
		}
	}
